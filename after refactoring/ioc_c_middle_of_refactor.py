...
#generate_IocXXX is used in generate_IOC_api(iocids)
def generate_IocSend_senderId(ioc, iocid, iocsenders, iocDataProperties):
    #sender data type
    input_para = ""
    Is_pointer = False
...
def generate_IocReceive(ioc, iocid, iocreceivers, iocDataProperties):
    #receiver data type
    input_para = ""
    Is_pointer = False
    iocType = config.getValue("/AUTOSAR/" + ioc + "/" + iocid + "/" + iocDataProperties[0] ,"OsIocDataTypeRef")
...
def generate_IocSendGroup(ioc, iocid, iocsenders, iocDataProperties):
    #sender data type
    i = 1
    input_para = ""
    Is_pointer = False
...
def generate_IocReceiveGroup(ioc, iocid, iocreceivers, iocDataProperties):
    i = 1
    input_para = ""
    Is_pointer = False
    for iocData in iocDataProperties:
...
def generate_IocWrite_senderId(ioc, iocid, iocsenders, iocDataProperties):
    i = 1
    data = ""
    Is_pointer = False
    for iocData in iocDataProperties:
        ...
        if "*" not in iocType:     
            ...
            if not(iocType.endswith("*")): #改動：type ref, uint16 length 情況
                ioctype_ptr = iocType.split(",")            
                data = ioctype_ptr[0] + " ptr_" + str(i) + ", " + "uint16 length_" + str(i) 
            else: #改動：type ref 情況
                data = iocType + " ptr_" + str(i)
            Is_pointer = True
        if iocData != iocDataProperties[-1]:
            data += ", "
        i += 1
...
def generate_IocRead_receiverId(ioc, iocid, iocreceivers, iocDataProperties):
    i = 1
    data = ""
    Is_pointer = False
    for iocData in iocDataProperties:
        ...
        if "*" not in iocType:     
            ...
            if not(iocType.endswith("*")): #改動：type ref, uint16 length 情況
                ioctype_ptr = iocType.split(",")            
                data = ioctype_ptr[0] + " ptr_" + str(i) + ", uint16* length_1"
            else: #改動：type ref 情況
                data = iocType + " ptr_" + str(i)
            Is_pointer = True
...
def generate_IocWriteGroup(ioc, iocid, iocsenders, iocDataProperties):
    i = 1
    input_para = ""
    length_index = []
    for iocData in iocDataProperties:
...
def generate_IocReadGroup(ioc, iocid, iocreceivers, iocDataProperties):
    i = 1
    data = ""
    Is_pointer = False
    for iocData in iocDataProperties:
    ...
    for receiver in iocreceivers:
        ...
        gr.printFile("#define OS_OSAPP_{}_START_SEC_CODE".format(app))
        gr.printFile("#include \"OS_MemMap.h\"")
        gr.printFile("Std_ReturnType IocReadGroup_{}_{}({})".format(iocid, receiver, data))
        gr.printFile("{")
        gr.printFile("    struct IOC_{}_struct d;".format(iocid))
        j = 1
        for iocdata in iocDataProperties:
        ...
        gr.printFile("#define OS_OSAPP_{}_STOP_SEC_CODE".format(app))
        gr.printFile("#include \"OS_MemMap.h\"")
        gr.printFile("Std_ReturnType SysIocReadGroup_{}_{}({})".format(iocid, receiver, data))
        gr.printFile("{")
        gr.printFile("    Std_ReturnType ret = IOC_E_OK;")
        gr.printFile("    if (ret == IOC_E_OK && (~(SystemObjectAutosar[_CoreID]->IocAutosar_r[{}].applicationsMask) & (1 << ExecutingApp[_CoreID])))".format(iocid))
        gr.printFile("    {")
...
def generate_IOC_api(ioc, iocids):
    for iocid in iocids:
        ...
        if length != False:
            ...
            if len(iocDataProperties) == 1:
                generate_IocSend_senderId(ioc, iocid, iocsenders, iocDataProperties)
                generate_IocReceive(ioc, iocid, iocreceivers, iocDataProperties)
            elif len(iocDataProperties) > 1:
                generate_IocSendGroup(ioc, iocid, iocsenders, iocDataProperties)
                generate_IocReceiveGroup(ioc, iocid, iocreceivers, iocDataProperties)
        else:
            if len(iocDataProperties) == 1:
                generate_IocWrite_senderId(ioc, iocid, iocsenders, iocDataProperties)
                generate_IocRead_receiverId(ioc, iocid, iocreceivers, iocDataProperties)
            elif len(iocDataProperties) > 1:
                generate_IocWriteGroup(ioc, iocid, iocsenders, iocDataProperties)
                generate_IocReadGroup(ioc, iocid, iocreceivers, iocDataProperties)
